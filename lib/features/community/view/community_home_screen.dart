...
class _CommunityHomeScreenState extends State<CommunityHomeScreen> {
  ...
  Future<void> _openPostDetail(Post post) async {
    final result = await openPostDetail<bool>(
      post.id,
      initialPost: post,
    );
...
class _PostCard extends StatelessWidget {
  ...
  const _PostCard({
    ...
    required this.obscureOwnerContent,
    required this.onTap,
  });

  @override
  Widget build(BuildContext context) {
    final hasImage = post.imagePaths.isNotEmpty && !obscureOwnerContent;
    final safeTitle = post.title.trim().isEmpty ? '(제목 없음)' : post.title.trim();
    final safeBody = post.body.trim();
                ...
                Expanded(
                  child: Text(
                    ...
                    style: TextStyle(
                      fontSize: 13,
                      height: 1.35,
                      color: obscureOwnerContent
                          ? const Color(0xFFA56E5F)
                          : const Color(0xFF4B5563),
                      fontWeight: obscureOwnerContent
                          ? FontWeight.w800
                          : FontWeight.w500,
                    ),
                  ),
