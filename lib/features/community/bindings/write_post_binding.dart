...
import 'package:yupgagae/features/community/data/firebase_post_image_repository.dart';
...
class WritePostBinding extends Bindings {
  ...
  void dependencies() {
    ...
    final tag =
        postId == null ? 'create:${initialBoardType.key}' : 'edit:$postId';

    if (!Get.isRegistered<PostImageRepository>()) {
      Get.lazyPut<PostImageRepository>(
    ...
    if (!Get.isRegistered<WritePostController>(tag: tag)) {
      Get.lazyPut<WritePostController>(
        () => WritePostController(
          ...
          imageRepo: Get.find<PostImageRepository>(),
