...
import 'package:yupgagae/features/my_store/data/server_backed_store_profile_repository.dart';
import 'package:yupgagae/features/my_store/domain/store_profile_repository.dart';

import 'package:yupgagae/features/notification/application/push_notification_service.dart';
import 'package:yupgagae/features/notification/data/cloud_functions_push_notification_repository.dart';
...
class RootBinding extends Bindings {
  ...
  void _bindStoreProfileRepository() {
    if (!Get.isRegistered<StoreProfileRepository>()) {
      final localRepository = InMemoryStoreProfileRepository(
  ...
  void _bindPushNotificationRepository() {
    if (!Get.isRegistered<PushNotificationRepository>()) {
      Get.put<PushNotificationRepository>(
    ...
    if (!Get.isRegistered<PushNotificationService>()) {
      Get.put<PushNotificationService>(
        PushNotificationService(
          repository: Get.find<PushNotificationRepository>(),
        ),
        permanent: true,
      );
    }
  ...
  void _startPushNotificationService() {
    final service = Get.find<PushNotificationService>();
    service.start();
  }
}